...
<%
	...
	String empid = Empid.substring(0, 1);
	ResultSet rs = null;
	if(empid.equals("P")){
		rs = DisplayDetails.Displayp(Empid);
	}
	else if(empid.equals("T")){
		rs = DisplayDetails.Displayt(Empid);
	}
...
<head>
  ...
  <div class="wrapper">
    ...
    <div class="title" style="margin: 7% 0 0 30%;"><h1><b>User Leave History</b></h1></div>
    <div class="card-body1" style="padding: 1.5rem;" >
      <div class="input-group" style= "padding : 0 0 0 0; margin: 0 0 0 0;">
			<% while(rs.next()) {%>		
					<h3 style="color:#433633;">ID &emsp;: <%=rs.getString("EmpId") %> <br>Name &emsp;: <%=rs.getString("name") %></h3><% } %>
				</div>
        ...
        <div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
          <div class="row">
            <div class="col-sm-12 col-md-6" style="left: 53%; ">
     		...
     		<div class="row" style="max-width: 103% ; width:103%;">
            <div class="col-sm-12" style="margin:0 0 0 5px;">
          ...
          <thead>
            <tr role="row" class="table-warning">
              ...
              <th class="sorting" style="width: 74.2px;">Request ID</th>
              <th class="sorting" style="width: 50.2px;">From</th>
              <th class="sorting" style="width: 74.2px;">Days</th>
              <th class="sorting" style="width: 74.2px;">Title</th>
              <th class="sorting" style="width: 50.2px;">Description</th>
              <th class="sorting" style="width: 50.2px;">Status</th>
          </thead>
          <tbody>
          <%
          int i = 1;
          	rs = DisplayLeaveReqDAO.DisplayLR(Empid);
          	while(rs.next()){
          %>
          <tr role="row" class="odd">
                <td class="align-middle sorting_1"><%=i++ %></td>
                <td class=" align-middle"><%=rs.getString("ReqId") %></td>
                <td class=" align-middle"><%=rs.getString("FromDate") %></td>
                <td class=" align-middle"><%=rs.getInt("Days") %></td>
                <td class=" align-middle"><%=rs.getString("Title") %></td>
                <td class=" align-middle"><%=rs.getString("Description") %></td>
                <td class=" align-middle"><%=rs.getString("Status") %></td>
            </tr>
            <%} %>
            </tbody>
        </table>
    ...
    <div>
    <a href="LeaveRAdminUser.jsp"><button  style="--color: #cfbab6; position:absolute; left:4%; bottom: 0 ;"><span>Back</span><i></i></button></a>
   </div>
    </div>
    </div>
    </div>
